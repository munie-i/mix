...
function PopularSlider() {
    ...
    useEffect(()=>{
        if (genre === 'movie') {
            fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${key}&include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc&without_genres=16`)
            .then(res=>res.json())
            .then(data=>setData(data.results));
            setType('movie')
        } else if (genre === 'tv') {
            fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${key}&include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc&without_genres=16`)
            .then(res=>res.json())
            .then(data=>setData(data.results));
            setType('tv')
        } else if (genre === 'animation') {
            fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${key}&include_adult=false&include_null_first_air_dates=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=16`)
            .then(res=>res.json())
            .then(data=>setData(data.results));
            setType('tv')
        }
